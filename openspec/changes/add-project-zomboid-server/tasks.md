...
- [ ] 5.5 Exercise and document graceful planned restart, crash restart behavior, scheduled backup creation/retention, controlled-update rollback behavior, and a local full-state restore validation. (Previous validation did not induce a failed Steam update; the failed-update path is now covered by a sandboxed maintenance test, with live validation still pending.)
