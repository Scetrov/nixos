...
**Reason**: Codex now returns a weekly-only window for the affected account, and provider primary/secondary slots do not guarantee 5-hour/7-day identities. Fixed panels can therefore relabel weekly data and fabricate missing capacity.

**Migration**: Replace both fixed-window stat panels with freshness-gated weekly remaining, weekly consumed trend, and weekly reset panels driven by duration-derived metrics.
