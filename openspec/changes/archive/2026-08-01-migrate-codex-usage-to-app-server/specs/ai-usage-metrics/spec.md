...
**Reason**: Direct private-endpoint polling and custom token refresh are the observed outage source and cannot correctly abstract rotating OAuth credentials or duration-dependent window semantics.

**Migration**: Codex collection moves to the local app-server `account/rateLimits/read` contract and the app-server runtime owns managed ChatGPT authentication and credential persistence.
