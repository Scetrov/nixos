{ config, pkgs, ... }:
let
  baseconfig = {
    allowUnfree = true;
  };
  unstable = import <nixos-unstable> { config = baseconfig; };
in
{
  age.secrets.scetrov-password.file = /root/secrets/scetrov_password.age;
  nix.settings.trusted-users = [
    "root"
    "scetrov"
  ];
  users.users.scetrov = {
    isNormalUser = true;
    description = "scetrov";
    passwordFile = config.age.secrets.passwordFile = config.age.secrets.scetrov-password.path;
    extraGroups = [
      "networkmanager"
      "wheel"
      "docker"
      "plugdev"
    ];
    openssh.authorizedKeys.keys = [
      "ssh-rsa "
    ];
    shell = pkgs.zsh;
    packages = with pkgs; [
      btop
      curl
      discord
      dotnetCorePackages.dotnet_9.sdk
      fira-code-nerdfont
      fzf
      git
      go
      insomnia
      jetbrains.rider
      lsb-release
      nixfmt-rfc-style
      nmap
      powershell
      rustup
      tmux
      unstable.brave
      unstable.chezmoi
      unstable.devenv
      unstable.foundry
      unstable.framesh
      unstable.ghostty
      unstable.hugo
      unstable.obsidian
      unstable.oh-my-posh
      usbutils
      vscode-fhs
      yubioath-flutter
    ];
  };
}
